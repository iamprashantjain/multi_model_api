...
user_menu = st.sidebar.selectbox('Select Options',('CV (CS)','CV (NCS)','2W (CS)','2W (NCS)','FE (CS)','FE (NCS)'))
...
if user_menu == "CV (CS)":
	...
	Fuel_Clean = col4.selectbox('Select State',fuel,index = 0)
	...
	fuel = df['Fuel_Clean'].loc[df['Variant_Clean'] == Variant_Clean].drop_duplicates().tolist()
	fuel.insert(0,'Select Fuel')
	Fuel_Clean = col6.selectbox('Select State',fuel,index = 0)


	METERREADING = col7.number_input('Enter Meter Reading',min_value=None)

	
	X = pd.DataFrame(columns=['MAKE_YEAR','Make_Clean','Model_Clean','Variant_Clean','Fuel_Clean','CV_State_Clean','METERREADING'],data=np.array([2013,'EICHER','380','AGRICULTURETRACTOR','DIESEL','HARYANA',2373]).reshape(1,7))
	...
	for i in uploaded_file:
		...
		if i is not None:
			...
			with st.spinner(f"Working... 💫"):
				...
				res = yolomodel(uploaded_image)
				st.subheader(f"Number Plate: {res}")