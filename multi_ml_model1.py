...
user_menu = st.sidebar.selectbox('Select Options',('CV (CS)','CV (NCS)','2W (CS)','2W (NCS)','FE (CS)','FE (NCS)','ANPR'))
...
if user_menu == "CV (CS)":
	...
	cust_seg = df['Customer_Segmentation'].loc[df['Variant_Clean'] == Variant_Clean].drop_duplicates().tolist()
	cust_seg.insert(0,'Select Segmentation')
	Customer_Segmentation = col7.selectbox('Select Segmentation',cust_seg,index = 0)
	...
	state = df['FE_State_Clean'].unique().tolist()
	state.insert(0,'Select Fuel')
	FE_State_Clean = col5.selectbox('Select State',state,index = 0)
